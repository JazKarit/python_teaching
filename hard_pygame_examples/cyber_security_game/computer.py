    ...
    def get_response(self,user_input):
        if self.is_locked and user_input == self.password and self.is_door:
            ...
            if user_input[3:] == 'office passwords':
                self.dir = 1
                return ["You are in the office passwords folder","Secure agencies must change their passwords every three month..."]
            elif user_input[3:] == 'python files':
                self.dir = 2
                return ["You are now in the python files folder"]
            elif user_input[3:] == 'door password':
                self.dir = 3
                return ["You are now in the door password folder"]
            elif user_input[3:] == '..':
                self.dir = 0
                return ["You are now in the main folder"]
            else:
                return ["Command not understood"]
